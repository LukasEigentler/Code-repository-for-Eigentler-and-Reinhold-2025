...
% This script calculates and visualises the bifurcation diagrams for an
% unbounded trait.
...
elseif parachange == "alpha2"
    if alt == 1
        % para_col = [0.01:0.01:0.74, 0.742:0.002:0.78, 0.79:0.01:2];
        para_col =0.1:0.01:2;
    else
...
elseif parachange == "gamma"
    para_col = 0:0.05:5; 
elseif parachange == "gamma_oomag"
    para_col = logspace(-1,4,100);
end
%% Parameters
...
filename = "num_sim_data/unbounded_trait_"+parachange+"_change"+strrep("_d"+num2str(d)+"_ph"+num2str(ph)+"_gamma"+num2str(gamma)+...
        "_alpha1"+num2str(alpha1)+"_alpha2"+num2str(alpha2)+"_m1"+num2str(m1)+"_m2"+num2str(m2)+alttext,'.','dot');
filename1 = "num_sim_data/unbounded_trait_mut_sel_balance_data_"+parachange+"_change"+strrep("_d"+num2str(d)+"_ph"+num2str(ph)+"_gamma"+num2str(gamma)+...
        "_alpha1"+num2str(alpha1)+"_alpha2"+num2str(alpha2)+"_m1"+num2str(m1)+"_m2"+num2str(m2)+alttext,'.','dot');
...
if plotonly == 0
    ...
    for mm = 1:length(para_col)
        ...
        elseif parachange == "d"
            d = para_col(mm);
        elseif parachange == "gamma" || parachange == "gamma_oomag"
            gamma = para_col(mm);
        elseif parachange == "p"
            ph = para_col(mm);
...
%         z=linspace(log(eps*alpha1/(1-alpha1*eps)),log((1-eps*alpha1)/(alpha1*eps)),M);
       ...
       u0 = 0.5*ones(1,length(z))/(z(end)-z(1));
        % if M>1 
...
if parachange == "d" || parachange == "gamma_oomag"
    set(gca,'xscale','log')
...
xlabel("Test")
if parachange == "m2"
    xlabel("Predator mortality, $m_2$", "Interpreter","latex")
elseif parachange == "alpha2"
...
elseif parachange == "d"
    xlabel("Prey mutation strength, $d$", "Interpreter","latex")
elseif parachange == "gamma"
    xlabel("Max predator growth rate, $\gamma$", "Interpreter","latex")
elseif parachange == "p"
    xlabel("Predation half saturation constant, $p$", "Interpreter","latex")
end
